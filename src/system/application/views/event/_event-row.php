...
<div class="row row-event">
	...
	<div class="text">
		...
		<p class="info"><strong><?php echo strtoupper($this->timezone->formattedEventDatetimeFromUnixtime($event->event_start, $event->event_tz_cont.'/'.$event->event_tz_place, 'd.M.Y')); ?></strong>
        <?php if ($event->event_start+86399 != $event->event_end) { ?>
        - <strong><?php echo strtoupper($this->timezone->formattedEventDatetimeFromUnixtime($event->event_end, $event->event_tz_cont.'/'.$event->event_tz_place, 'd.M.Y')); ?></strong> at <strong><?php echo escape($event->event_loc); ?></strong>
        <?php } ?>
        </p>
    	<div class="desc">
	...
	?>
		...
		<?php if(isset($event->is_cfp) && $event->is_cfp): ?>
			Call for papers ends <b><?php echo strtoupper(date('d.M.Y',$event->event_cfp_end)); ?></b>
