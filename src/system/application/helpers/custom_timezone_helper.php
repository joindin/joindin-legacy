...
function custom_timezone_menu($name, $valueCont = null, $valuePlace = null)
{
	...
	$tzs = array();
	$validConts = array('Africa', 'America', 'Antarctica', 'Arctic', 'Asia', 'Atlantic', 'Australia', 'Europe', 'Indian', 'Pacific');
	...
	$str.= '<select onchange="change_cont_'.$hname.'(this);" name="'.$hname.'_cont" id="'.$hname.'_cont">';
...
EOT;
	foreach ($conts as $cont) {
		...
		foreach ($tzs[$cont] as $num=>$place) {
			$str.= 'placeSelect.options[placeSelect.options.length] = new Option("'.htmlentities($place).'", "'.htmlentities($place).'");'."\n";
			if ($valuePlace == $place) {
				$str.= 'placeSelect.options[placeSelect.options.length - 1].selected=true;'."\n";
			}
		}
		$str.= 'break;'."\n";
	}
