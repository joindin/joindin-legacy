...
class Event extends Controller
{
    ...
    function calendar($year = null, $month = null, $day = null)
    {
        ...
        $end    = mktime(23, 59, 59, $month, $day === null
            ? date('t', $start) : $day, $year);
    ...
    function view($id, $opt = null, $opt_id = null)
    {
        ...
        if ($this->validation->run() != true) {
            ...
            if (!$is_auth) {
                $def_ret = $this->defensio->check('Anonymous', $ec['comment'],
                    $is_auth, '/event/view/' . $id);
            ...
            redirect('event/view/' . $events[0]->ID . '#comments',
                'location', 302);
        }
        ...
        if (!$is_auth) {
            $info = array(
                'msg' => sprintf(' <h4 style="color:#3A74C5">New to ' .
        ...
        if (count($attend) > 0) {
            $this->template->write_view('sidebar3',
        ...
        if ($arr['admin']) {
            $this->template->write_view('sidebar2', 'event/_sidebar-admin',
                array(
                    'eid'         => $id,
                    'is_private'  => $events[0]->private,
                    'evt_admin'   => $this->event_model->getEventAdmins($id),
                    'claim_count' => count($this->uadm->
                        getPendingClaims_Talks($id))
                ));
        }

        $this->template->write_view('content', 'event/detail', $arr, true);
        $this->template->write_view('sidebar2', 'event/_event_contact', array(
            'eid' => $id
        ));
        $this->template->render();
    ...
    function ical($id)
    {
        ...
        $this->load->view('event/ical', array(
            'data' => $arr
        ));
    }
    ...
    function codes($id)
    {
        ...
        $cl = $this->event_model->getClaimedTalks($id,
            $talks); //echo '<pre>'; print_r($cl); echo '</pre>';
        foreach ($cl as $k => $v) {
            ...
            foreach ($sp as $spk => $spv) {
                $code = buildCode($v->rid, $v->tdata['event_id'],
                    $v->tdata['talk_title'], trim($spv));
                if ($code == $v->rcode) {
                    $cl[$k]->code = $code;
                }
            }
    ...
    function submit()
    {
        ...
        $fields = array(
            ...
            'event_desc' => 'Event Description',
            'start_mo' => 'Event Start Month', 'start_day' => 'Event Start Day',
            'start_yr' => 'Event Start Year', 'is_cfp' => 'Is CfP',
            'cfp_start_day' => 'CfP Start Day',
            'cfp_start_mo' => 'CfP Start Month',
            'cfp_start_yr' => 'CfP Start Year', 'cfp_end_day' => 'CfP End Day',
            'cfp_end_mo' => 'CfP End Month', 'cfp_end_yr' => 'CfP End Year',
            'end_mo' => 'Event End Month', 'end_day' => 'Event End Day',
            'end_yr' => 'Event End Year', 'event_loc' => 'Event Location',
            'event_tz_cont' => 'Event Timezone (Continent)',
            'event_tz_place' => 'Event Timezone (Place)',
            'event_stub' => 'Event Stub'//	'cinput'				=> 'Captcha'
        );
        $rules = array(
            'event_title' => 'required|callback_event_title_check',
            'event_loc' => 'required', 'event_contact_name' => 'required',
            'event_contact_email' => 'required|valid_email',
            'event_tz_cont' => 'required', 'event_tz_place' => 'required',
            'start_mo' => 'callback_start_mo_check',
            'end_mo' => 'callback_end_mo_check',
            'cfp_start_mo' => 'callback_cfp_start_mo_check',
            'cfp_end_mo' => 'callback_cfp_end_mo_check',
            'event_stub' => 'callback_stub_check', 'event_desc' => 'required',
        ...
        if (empty($this->validation->start_mo)) {
            $sel_fields = array(
                'start_mo' => 'm', 'start_day' => 'd', 'start_yr' => 'Y',
                'end_mo' => 'm', 'end_day' => 'd', 'end_yr' => 'Y',
                'cfp_start_mo' => 'm', 'cfp_start_day' => 'd',
                'cfp_start_yr' => 'Y', 'cfp_end_mo' => 'm',
                'cfp_end_day' => 'd', 'cfp_end_yr' => 'Y'
            );
        ...
        if ($this->validation->run() != FALSE) {
            ...
            $startUnixTimestamp = $this->timezone->UnixtimeForTimeInTimezone($tz,
                $this->input->post('start_yr'), $this->input->post('start_mo'),
                $this->input->post('start_day'), 0, 0, 0);

            // Get offset unix timestamp for end of event
            $endUnixTimestamp = $this->timezone->UnixtimeForTimeInTimezone($tz,
                $this->input->post('end_yr'), $this->input->post('end_mo'),
                $this->input->post('end_day'), 23, 59, 59);
            ...
            $sub_arr = array(
                ...
                'event_lat' => $this->input->post('event_lat'),
                'event_long' => $this->input->post('event_long'),
                'event_desc' => $this->input->post('event_desc'), 'active' => 0,
                'event_stub' => $this->input->post('event_stub'),
                'event_tz_cont' => $this->input->post('event_tz_cont'),
                'event_tz_place' => $this->input->post('event_tz_place'),
                'pending' => 1, 'private' =>
                ($this->input->post('is_private') == 'n') ? null
            ...
            if (!empty($cfp_check)) {
                ...
                $sub_arr['event_cfp_start'
                ] = $this->timezone->UnixtimeForTimeInTimezone($tz,
            ...
            $def = $this->defensio->check($cname, $ccomment, $is_auth,
                '/event/submit');
            ...
            if ($is_spam != 'true') {
                ...
                $subj =
                    'Event submission from ' . $this->config->item('site_name');
                $msg = 'Event Title: ' . $this->input->post('event_title') .
                    "\n\n";
                $msg .=
                    'Event Description: ' . $this->input->post('event_desc') .
                        "\n\n";
                $msg .= 'Event Date: ' .
                ...
                $msg .= 'Spam check: ' . ($is_spam == 'false') ? 'not spam'
                    : 'spam';
                ...
                $admin_emails = $this->user_model->getSiteAdminEmail();
                foreach ($admin_emails as $user) {
                    mail($user->email, $subj, $msg,
                        'From: ' . $this->config->item('email_submissions'));
                }
                $arr['msg'] = sprintf('
                ...
                $arr['msg'] =
                    'There was an error submitting your event! Please <a href="' .
    ...
    function export($eid)
    {
        ...
        header('Content-Disposition: attachment; filename="Event_Comments_' .
            $eid . '.csv"');
        echo $out;
    }
    ...
    {
        if ($this->user_model->isSiteAdmin() ||
            $this->user_model->isAdminEvent($eid)) {
            //they're okay
        } else {
            redirect('event/view/' . $eid);
        }
            ...
            foreach ($claim as $k => $v) {
                ...
                switch (strtolower($v)) {
                    case 'approve':
                        ...
                        $this->sendemail->claimSuccess($email, $talk_title,
                            $evt_name);

                        $approved++;
                        break;
                    case 'deny':
                        $this->db->delete('user_admin', array(
                            'ID' => $k
                        ));
                        $denied++;
                        break;
            ...
            foreach ($posted_claims as $uam_key => $claim) {
                switch (strtolower($claim)) {
                    case 'approve':
                        //Approve the claim
                        echo 'approve';
                        $this->uam->updatePerm($uam_key, array(
                            'rcode' => ''
                        ));
                        break;
                    case 'deny':
    ...
    function import($eid)
    {
        ...
        if (!empty($_POST) && $this->upload->do_upload('xml_file')) {
            ...
            try {
                ...
                $msg = 'Import Successful! <a href="/event/view/' . $eid .
                    '">View event</a>';
        ...
        $arr = array(
            'details' => $evt_detail, 'msg' => $msg, 'error_msg' => $error_msg
        );
    ...
    function invite($eid, $resp = null)
    {
        ...
        if ($resp && $user) {
            switch (strtolower($resp)) {
                ...
                case "request":
                    ...
                    $evt_id = $detail[0]->ID;
                    $this->sendemail->sendInviteRequest($evt_id, $evt_title,
                        $user, $admins);
                    ...
                    $this->template->write_view('content', 'event/request',
                        $arr);
                    $this->template->render();
                    return;
                    break;
                ...
                if (empty($ret)) {
                    ...
                    if (!$is_invited) {
                        ...
                        $this->sendemail->sendInvite(
                            $ret[0]->email, $eid, $detail[0]->event_name);
                        $msg =
                            'User <b>' . $u . '</b> has been sent an invite!';
                    } else {
                        $msg = 'User <b>' . $u .
        ...
        $arr = array(
            'eid' => $eid, 'invites' => $invites, 'msg' => $msg,
            'evt_detail' => $detail
        );
    ...
    function contact($eid)
    {
        ...
        if ($this->validation->run() != FALSE) {
            ...
            $this->sendemail->sendEventContact($eid, $evt_name, $msg, $user,
                $admins);
    ...
    function event_title_check($str)
    {
        ...
        if (isset($ret[0]->id)) {
            $this->validation->set_message('event_title_check',
    ...
    function start_mo_check($str)
    {
        ...
        $t = mktime(0, 0, 0, $this->validation->start_mo,
            $this->validation->start_day, $this->validation->start_yr);
        $e = mktime(0, 0, 0, $this->validation->end_mo,
            $this->validation->end_day, $this->validation->end_yr);
        if ($t > $e) {
            $this->validation->set_message('start_mo_check',
    ...
    {
        $st = mktime(0, 0, 0, $this->validation->start_mo,
            $this->validation->start_day, $this->validation->start_yr);
        $et = mktime(23, 59, 59, $this->validation->end_mo,
            $this->validation->end_day, $this->validation->end_yr);
        if ($et < $st) {
            $this->validation->set_message('end_mo_check',
    ...
    {
        $cfp_st = mktime(0, 0, 0, $this->validation->cfp_start_mo,
            $this->validation->cfp_start_day, $this->validation->cfp_start_yr);
        $cfp_end = mktime(0, 0, 0, $this->validation->cfp_end_mo,
            $this->validation->cfp_end_day, $this->validation->cfp_end_yr);
        $evt_st = mktime(0, 0, 0, $this->validation->start_mo,
            $this->validation->start_day, $this->validation->start_yr);
        if ($cfp_st >= $evt_st) {
            $this->validation->set_message('cfp_start_mo_check',
                'Call for Papers must start before the event!');
            return false;
        }
        if ($cfp_st >= $cfp_end) {
            $this->validation->set_message('cfp_start_mo_check',
                'Invalid Call for Papers start date!');
            return false;
        }
        return true;
    }
    ...
    function cfp_end_mo_check()
    {
        ...
        if ($cfp_end >= $evt_st) {
            $this->validation->set_message('cfp_start_mo_check',
                'Invalid Call for Papers end date! CfP must end before event start!');
            return false;
        }
    }
    ...
    function chk_email_check($str)
    {
        ...
        if ($val == 1 && !$this->validation->valid_email($str)) {
            $this->validation->set_message('chk_email_check',
    ...
    {
        if ($this->input->post('cinput') !=
            $this->session->userdata('cinput')) {
    ...
    function stub_check($str)
    {
        if (!empty($str)) {
            $this->load->model('event_model');
            $id = ($this->uri->segment(3)===false) ? null
                : $this->uri->segment(3);

            $ret = $this->event_model->isUniqueStub($str, $id);
            if (!$ret) {
                $this->validation->set_message('stub_check',
