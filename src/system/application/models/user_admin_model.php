...
class User_Admin_Model extends Model
{
    ...
    {
        $params = array('uid' => $uid, 'rid' => $rid, 'rtype' => $rtype);
    ...
    /**
     * Given the ID from the user_admin table,
     * check to see if the claim is valid & pending
     *
    ...
    {
        $param = array('ID' => $claim_id, 'rcode' => 'pending');
        $q = $this->db->get_where('user_admin', $param);
        $ret = $q->result();
        return (empty($ret)) ? false : true;
    }
    ...
    public function getPendingClaims($type = 'talk', $rid = null)
    {
        switch ($type) {
        case 'talk':
            return $this->getPendingClaimsTalks($rid);
            break;
        case 'event':
            return $this->getPendingClaimsEvents($rid);
            break;
        }
    }
     ...
     *
     * @param integer $eid [optional] Event ID to restrict on
     *
     * @return array 
     */
    public function getPendingClaimsTalks($eid = null)
    {
    ...
    /**
     * Get pending claims for events
     * 
     * @param integer $eid event id
     *
     * @return array
     */
    public function getPendingClaimsEvents($eid = null)
    {
        $addl = ($eid) ? ' e.ID=' . $this->db->escape($eid) . ' and ' : '';
        $sql = sprintf(
            "select
