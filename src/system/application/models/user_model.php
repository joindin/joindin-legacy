...
class User_Model extends Model
{
     ...
     * @param boolean $plaintxt Flag to treat password as plaintext or md5
     *
     * @return boolean
     */
    ...
    /**
     * Output "logged in"/"logged out" HTML for the template based on login status
     ...
     *
     * @param string $user User ID/username
     *
    ...
    public function isAdminEvent($eid, $uid = null)
    {
        ...
        $params = array('uid' => $uid, 'rid' => $eid, 'rtype' => 'event');
        $q = $this->db->get_where('user_admin', $params);
        $ret = $q->result();
    ...
    public function isAdminTalk($tid)
    {
        if ($this->isAuth()) {
            $ad = false;
            $uid = $this->session->userdata('ID');
            $params = array('uid' => $uid, 'rid' => $tid, 'rtype' => 'talk');
            $q = $this->db->get_where('user_admin', $params);
            $ret = $q->result();
            ...
            if (isset($ret[0]->event_id) && $this->isAdminEvent($ret[0]->event_id)) {
                $ad = true;
            }
            return $ad;
        } else {
    ...
    {
        $u = $this->getUser((int)$uid);
        $up = ($u[0]->active == 1) ? array('active' => '0') : array('active' => '1');
    ...
    {
        $u = $this->getUser((int)$uid);
        $up = ($u[0]->admin == 1) ? array('admin' => null) : array('admin' => '1');
        $this->updateUserinfo($uid, $up);
    }
     ...
     *
     * @param mixed $in User ID or Username
     *
     ...
     *
     * A reduced version of the getUser() method so we can safely
     * return these results to the service.
    ...
    {
        $this->db->select('email')->where('admin', 1);
        $q = $this->db->get('user');
        return $q->result();
    }
    ...
    /**
     * Find other users of the system
     * that were speakers at events the given user was a speaker at too
     *
     * @param integer $uid   User ID
     * @param integer $limit [optional] integer Limit the number of results returned
     *
     * @return array Return user info array (user_id, event_id, username, full_name)
     */
    public function getOtherUserAtEvt($uid, $limit = 15)
    {
        if (!ctype_digit((string)$limit)) {
            throw new Exception('Expected $limit to be a number, received '.$limit);
        }

        //find speakers (users attending too?)
        // that have spoken at conferences this speaker did too
     ...
     * @param integer $start [optional] Starting point for search (not used)
     * @param integer $end   [optional] Ending point for search (not used)
     *
     * @return array
     */
        ...
        $sql = sprintf(
            "select
                ...
                (select count(ID)
                    from user_admin
                    where rtype='talk'
                    and uid=u.ID) talk_count,
                (select count(ID)
                    from user_attend
                    where uid=u.ID) event_count
            from
                user u
            where
