...
class Talk extends MY_Controller
{
    ...
    function add($id = null, $opt = null)
    {
        ...
        $rules = array(
            'event_id','Event Name','required',
            'talk_title','Talk Title','required',
            'talk_desc','Talk Description','required',
            'session_type','Session Type','required',
            'session_lang','Session Language','required',
            'given_mo','Given Month','callback_given_mo_check'
        );
        ...
        if ($id) {
            ...
            $this->form_validation->session_track = (empty($track_info))
            ...
            $this->form_validation->speaker
                = $this->talkSpeakers->getSpeakerByTalkId($id);

            $this->form_validation->eid       = $thisTalk->eid;
            $this->form_validation->given_day = $this->timezone
                ->formattedEventDatetimeFromUnixtime(
                    $thisTalk->date_given,
                    $thisTalk->event_tz_cont . '/' . $thisTalk->event_tz_place,
                    'd'
                );
            $this->form_validation->given_mo = $this->timezone
                ->formattedEventDatetimeFromUnixtime(
                    $thisTalk->date_given,
                    $thisTalk->event_tz_cont . '/' . $thisTalk->event_tz_place,
                    'm'
                );
            $this->form_validation->given_yr = $this->timezone
                ->formattedEventDatetimeFromUnixtime(
                    $thisTalk->date_given,
                    $thisTalk->event_tz_cont . '/' . $thisTalk->event_tz_place,
                    'Y'
                );
            $this->form_validation->given_hour = $this->timezone
                ->formattedEventDatetimeFromUnixtime(
                    $thisTalk->date_given,
                    $thisTalk->event_tz_cont . '/' . $thisTalk->event_tz_place,
                    'H'
                );
            $this->form_validation->given_min = $this->timezone
                ->formattedEventDatetimeFromUnixtime(
                    $thisTalk->date_given,
                    $thisTalk->event_tz_cont . '/' . $thisTalk->event_tz_place,
                    'i'
            ...
            $this->form_validation->talkDate = $this->form_validation->given_yr . '-' .
                $this->form_validation->given_mo . '-' .
                $this->form_validation->given_day;

            $this->form_validation->session_lang = $thisTalk->lang_id;
            $this->form_validation->session_type = $thisTalk->tcid;
        } else {
            ...
            $this->form_validation->given_mo   = date('m', $thisTalksEvent->event_start);
            $this->form_validation->given_day  = date('d', $thisTalksEvent->event_start);
            $this->form_validation->given_yr   = date('Y', $thisTalksEvent->event_start);
            $this->form_validation->given_hour = date('H', $thisTalksEvent->event_start);
            $this->form_validation->given_min  = date('i', $thisTalksEvent->event_start);

            $this->form_validation->session_track = null;
            $this->form_validation->speaker       = array();
            ...
            if ($posted_speakers = $this->input->post('speaker_row')) {
                foreach ($posted_speakers as $speaker) {
                    ...
                    $this->form_validation->speaker[] = $obj;
                    unset($obj);
                }
            }
        ...
        if (isset($eid)) {
            $this->form_validation->event_id = $eid;
        }
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($id) {
                ...
                $this->form_validation->speaker = $this->talkSpeakers
                    ->getTalkSpeakers($id);
                ...
                if ($this->input->post('session_track') != 'none') {
                    ...
                    $this->form_validation->session_track = $new_track;
    ...
    function given_mo_check($str)
    {
        ...
        $det = $this->event_model->getEventDetail($this->form_validation->event_id);
        $thisTalk = $det[0];
