...
class Speaker extends MY_Controller
{
    ...
    public function edit()
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($up_err) {
                $this->form_validation->error_string = $up_err;
            } elseif (isset($cdata[0])) {
                $this->sp->updateProfile($udata[0]->ID, $data);
                $this->form_validation->error_string = 'Profile successfully updated!';
            } else {
                $this->sp->setProfile($data);
                $this->form_validation->error_string = 'Profile successfully saved!';
            }

            $this->form_validation->error_string .= '<br/>Please ' .
                '<a href="/speaker/profile">click here</a> ' .
                'to return to your speaker profile';
        } else {
            // If there's not an data set, get from their profile
            if (empty($this->form_validation->email)) {
                $udata = $this->user_model->getUser(
                    $this->session->userdata('ID')
                );
                $this->form_validation->email     = $udata[0]->email;
                $this->form_validation->full_name = $udata[0]->full_name;
            }
        }
        ...
        if (isset($p[3])) {
            ...
            case 'edit':
                ...
                if ($req_type == 'edit') {
                    ...
                    $this->form_validation->token_name = $token->access_token;
                    $this->form_validation->token_desc = $token->description;
                    $this->form_validation->is_public  = $token->is_public;
                }
