...
class Blog extends MY_Controller
{
    ...
    function add($id = null)
    {
        ...
        $rules = array(
            ...
            'story','Entry Content','required',
            'post_mo','Post Month','required',
            'post_day','Post Day','required',
            'post_yr','Post Year','required',
            'post_hr','Post Hour','required',
            'post_mi','Post Minute','required',
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($id) {
                ...
                $this->form_validation->title = $det[0]->title;
                $this->form_validation->story = $det[0]->content;

                $this->form_validation->post_mo = date('m', $det[0]->date_posted);
                $this->form_validation->post_day = date('d', $det[0]->date_posted);
                $this->form_validation->post_year = date('Y', $det[0]->date_posted);

                $this->form_validation->post_hr = date('H', $det[0]->date_posted);
                $this->form_validation->post_mi = date('i', $det[0]->date_posted);
            }
        }
